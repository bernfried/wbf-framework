  ...
  private void createActionSettings(CoaSession session, Hashtable<String, String> attr, TemplateDefinition td) {
    ...
    while(actionKeys.hasMoreElements()) {
      ...
      while(tdForwardKeys.hasMoreElements()) {
        ...
        if (afDef.getType() == ActionForwardDef.ACTION_FORWARD_TYPE_CALL_XCHG_WEBLET || afDef.getType() == ActionForwardDef.ACTION_FORWARD_TYPE_CALL_XCHG_WEBLET_TO_REFERER
            ...
            while(ctntDatas.hasNext()) {
              ...
              if (dataAsText != null) {
                ...
                if (cs.existsContent(session, targetProject, ctntPath, ctntName)) {
                  if (newCtnt == null)
                    newCtnt = cs.getContent(session, targetProject, ctntPath, ctntName);
                  ...
                  if (result != 0) {
                    // content has no data
                    log.debug(LOG_CATEGORY_GENERATOR,
                        "ActionGenerateTemplateDefinitionInstance - executeTemplates: content '" + targetProject + " - " + ctntPath + "/" + ctntName + " (" + data.getLocale()
